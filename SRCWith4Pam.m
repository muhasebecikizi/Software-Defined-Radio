...
% cdi=0 %input('channel multipath: 0 for none, 1 for mild or 2 for harsh ::  ');
% fo =0.01 %input('tranmsitter mixer freq offset in %: try 0 or 0.01 ::  ');  
% po =0 %input('tranmsitter mixer phase offset in rad: try 0, 0.7 or 0.9 ::  ');
...
% 
% Len_sound=length(y); % total audio length
% 
% sound(y,Fs); % plays audio files
% t_sound = [0:1/Fs:(length(y)/Fs)-1/Fs]; % total audio length in seconds
...
% quantization = (max(mono_y)-(min(mono_y)))/(2^(bit_num)) 
...
% end
% 
% quantization_error=sum(errq.*errq)/Len_sound
% %stem(errq)
% 
% 
% %Plot quantized
...
% hold on
% plot(t_sound,mono_y);
% stairs(t_sound,y_quantized*(quantization)); 
% 
...
% binaryo = [dec2bin(sn) dec2bin(abs(y_quantized),bit_num)];
% 
% 
% 
% 
% sound_bin_arr = zeros(1,(bit_num+2)*Len_sound);
% 
...
% for i = 1 : Len_sound
%     for j = 1 : (bit_num+2)
%         sound_bin_arr(((i-1)*(bit_num+2)) + (j)) = binaryo(i,j)-48; 
%     end
% end
% 
% 
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % encode 
% coded_sound = blockcode52_encode(sound_bin_arr);
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % biner to 4-PAM
% j=1; mpam=zeros(1,ceil(length(coded_sound)/2)); 
% for i=1:2:length(coded_sound)-1                 % and then into 4-PAM
%   if coded_sound(i:i+1)==[0,0], mpam(j)=-3; end 
%   if coded_sound(i:i+1)==[0,1], mpam(j)=-1; end
%   if coded_sound(i:i+1)==[1,0], mpam(j)=1; end
%   if coded_sound(i:i+1)==[1,1], mpam(j)=3; end
%   j=j+1;
% end
% 
% % output mpam
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% mpam_len=length(mpam);    % 4-level signal of length N
% %zero pad T-spaced symbol sequence to create upsampled T/M-spaced
% %sequence of scaled T-spaced pulses (with T = 1 time unit)
% M=200-so; mup=zeros(1,mpam_len*M); mup(1:M:end)=mpam; % oversampling factor
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %SRRC pulse filter with T/M-spaced impulse response
% srrc_L=0.5; p=(14.0)*srrc(srrc_L,0.3,M,0.4);  % blip pulse of width M
% x=filter(p,1,mup);                  % convolve pulse shape with data
% figure(2), plotspec(x,1/M)          % baseband signal spectrum
% % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % am modulation
...
% flat_fading=[ones(1,floor(0.2*lr)),0.5*ones(1,lr-floor(0.2*lr))]; % flat fading profile
% r=r.*flat_fading;               % apply profile to transmitted signal vector
%  
...
% AGC_gain=zeros(1,lr); AGC_gain(1)=1;          % initialize gain
% nr=zeros(1,lr);
% AGC_mu=0.0003;                      % stepsize
% for i=1:lr-1
%  nr(i)=AGC_gain(i)*r(i);               % AGC output
%  AGC_gain(i+1)=AGC_gain(i)-AGC_mu*(nr(i)^2-ds);   % adapt gain
% end
...
% theta=zeros(1,length(t_mod)); theta(1)=0;     % initialize estimate vector
% zs=zeros(1,fl+1); zc=zeros(1,fl+1);       % initialize buffers for LPFs
% for k=1:length(t_mod)-1                       % z's contain past fl+1 inputs
...
% ram = rnv;
% c2=cos(2*pi*fc*t_mod + phoff);                   % synchronized cosine for mixing
...
% length(v)
% plot(v)
figure(8), ul=floor((length(v)-124)/(4*rM)); 
plot(reshape(v(125:ul*4*rM+124),4*rM,ul)) % plot the eye diagram
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% clock recovery algorithm
% xcl = v; clock_n =mpam_len ; clock_L = srrc_L; 
% tnow=clock_L*M+1; tau=0; xs=zeros(1,clock_n);          % initialize variables
% tausave=zeros(1,clock_n); tausave(1)=tau; i=0;
...
% end
% figure(9), subplot(2,1,1), plot(xs(1:i-2),'b.')    % plot constellation diagram
...
% figure(10), plot([1:length(z)],z,'.')      % soft decisions
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
